...
void startup()
{
	...
	content.LoadGLTF("assets/glock.gltf");
...
void update()
{
	// for shooting the gun when q is pressed and Pressed == false rotate back and turn pressed to true
	if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS && Pressed == false) {
			modelRotation.y = 0.5;
			Pressed = true;
	}

	// Flick back to starting position return to false 
	if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_RELEASE && Pressed == true) {
		modelRotation.y = -0.1f;
		Pressed = false;
	}

	//Model rotation
	if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) { modelRotation.y += 0.01f; }
	if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) {modelRotation.y -= 0.01f; }
 	if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {modelRotation.x += 0.01f; }
	if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) { modelRotation.x -= 0.01f; }
